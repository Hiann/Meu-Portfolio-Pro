...
function Counter({ target, suffix = '' }: { target: number; suffix?: string }) {
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => { c += step; if (c >= target) { setN(target); clearInterval(id); } else setN(c); }, 18);
...
/* ─── TypeWriter ────────────────────────────────────────────────────────── */
function TypeWriter({ words, speed = 85, pause = 1800 }: { words: string[]; speed?: number; pause?: number }) {
  ...
  return <span className="tw-text">{display}<span className="tw-cursor">|</span></span>;
}
  ...
  return (
    <div ref={ref} className="skill-row">
      ...
      <div className="skill-track">
        <div className="skill-fill" style={{ width: inView ? `${level}%` : '0%', transitionDelay: `${delay}ms` }} />
      </div>
    </div>
  );
}
...
function GlitchText({ text, dim = false }: { text: string; dim?: boolean }) {
  ...
  useEffect(() => {
    ...
    const go = () => { t = setTimeout(() => { setGlitch(true); setTimeout(() => { setGlitch(false); go(); }, 160); }, 5000 + Math.random() * 3000); };
    go(); return () => clearTimeout(t);
  }, []);
  return (
    <span className={`glitch-text${dim ? ' dim' : ''}${glitch ? ' glitching' : ''}`}>{text}</span>
  );
}
...
function Terminal() {
  const LINES = [
    ...
    { p: '>>> hiann.skills', o: "['Python', 'AI/ML', 'Java', 'SQL', 'MongoDB', 'Git']" },
...
function ContactForm() {
  ...
  const validate = (f = form) => {
    ...
    else if (f.msg.trim().length < 10) errs.msg = 'Mensagem muito curta (mín. 10 caracteres)';
    return errs;
  };
  ...
  useEffect(() => {
    ...
    const tick = setInterval(() => {
      setCountdown(c => {
        if (c <= 1) { clearInterval(tick); reset(); return 0; }
        return c - 1;
      });
    }, 1000);
  ...
  if (status === 'sent') return (
    <div className="form-success" style={{ height: '100%', justifyContent: 'center' }}>
      <div className="form-success-top">
        <div className="form-success-icon">
          ...
          <svg className="form-success-ring" viewBox="0 0 52 52">
            <circle cx="26" cy="26" r="23" fill="none" stroke="rgba(var(--accent-rgb),.12)" strokeWidth="1.5"/>
            <circle cx="26" cy="26" r="23" fill="none" stroke="var(--accent)" strokeWidth="1.5"
      ...
      <div style={{
        ...
        background: 'rgba(var(--accent-rgb), 0.03)', border: '1px dashed var(--border-2)',
        display: 'flex', alignItems: 'center', justifyContent: 'space-between', textAlign: 'left'
      }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', minWidth: 0 }}>
          <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: '8px', fontWeight: 700, letterSpacing: '0.2em', textTransform: 'uppercase', color: 'var(--text-3)' }}>
            Email Vinculado
          </span>
          <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: '11px', color: 'var(--text-1)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
            {form.email}
          </span>
        </div>
        <svg width="16" height="16" fill="none" stroke="var(--accent)" strokeWidth="2" viewBox="0 0 24 24" style={{ flexShrink: 0, marginLeft: '8px' }}>
  ...
  if (status === 'error') return (
    <div className="form-error" style={{ height: '100%', justifyContent: 'center' }}>
      ...
      <p className="form-error-sub">Verifique sua conexão e tente novamente.</p>
      <button className="form-retry" onClick={() => setStatus('idle')}>Tentar novamente →</button>
    </div>
  );
  ...
  return (
    <div className="form-body" style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      
      {/* ─── Destinatário Fixo ─── */}
      <div className="form-field" style={{ marginBottom: '8px' }}>
        <label className="form-label" style={{ color: 'var(--accent)' }}>Destinatário</label>
        <div style={{
          display: 'flex', alignItems: 'center', gap: '8px',
          padding: '10px 4px', borderBottom: '1.5px dashed var(--accent)',
          opacity: 0.8, cursor: 'not-allowed', backgroundColor: 'rgba(var(--accent-rgb), 0.02)'
        }}>
          <svg width="12" height="12" fill="none" stroke="var(--accent)" strokeWidth="2" viewBox="0 0 24 24">
            <rect x="2" y="4" width="20" height="16" rx="2"/>
            <path d="M2 4l10 8 10-8"/>
          </svg>
          <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: '11px', color: 'var(--text-1)' }}>hiannpdr1234@gmail.com</span>
          <span style={{ marginLeft: 'auto', fontSize: '8px', color: 'var(--accent)', letterSpacing: '0.1em', fontWeight: 'bold' }}>[LOCKED]</span>
        </div>
      </div>
      ...
      <div className="form-field">
        ...
        <input type="text" placeholder="Seu nome completo" value={form.name}
          ...
          onChange={e => { setForm(p => ({ ...p, name: e.target.value })); if (touched.name) setErrors(validate({ ...form, name: e.target.value })); }} />
      </div>
      ...
      <div className="form-field" style={{ marginTop: '0.25rem' }}>
        ...
        <input type="email" placeholder="seu@email.com" value={form.email}
          ...
          onChange={e => { setForm(p => ({ ...p, email: e.target.value })); if (touched.email) setErrors(validate({ ...form, email: e.target.value })); }} />
      </div>
      ...
      <div className="form-field" style={{ marginTop: '0.25rem', flexGrow: 1 }}>
        ...
        <textarea placeholder="Descreva seu projeto ou proposta..."
          value={form.msg}
          ...
          onChange={e => { setForm(p => ({ ...p, msg: e.target.value })); if (touched.msg) setErrors(validate({ ...form, msg: e.target.value })); }} />
      </div>
      
      <button className="form-submit" disabled={status === 'sending'} onClick={handleSubmit}
        style={{ marginTop: 'auto', opacity: !canSubmit && Object.keys(touched).length > 0 ? 0.6 : 1 }}>
        {status === 'sending' ? (
  ...
  return (
    <button className="theme-toggle" onClick={onToggle} aria-label="Alternar tema">
      <div className="toggle-track">
        <svg className="toggle-track-sun" viewBox="0 0 24 24" fill="none"
          stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"
          style={{ color: dark ? 'rgba(var(--accent-rgb),.3)' : 'var(--accent)', opacity: dark ? 0.35 : 1 }}>
          <circle cx="12" cy="12" r="4"/>
          <path d="M12 2v2M12 20v2M4.22 4.22l1.42 1.42M18.36 18.36l1.42 1.42M2 12h2M20 12h2M4.22 19.78l1.42-1.42M18.36 5.64l1.42-1.42"/>
        </svg>
        <svg className="toggle-track-moon" viewBox="0 0 24 24" fill="none"
          stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"
          style={{ color: dark ? 'var(--accent)' : 'rgba(var(--accent-rgb),.3)', opacity: dark ? 1 : 0.35 }}>
...
const REPO_META: Record<string, { title?: string; desc?: string; tags?: string[] }> = {
  'IA-Analise-Veterinaria':      { title: 'IA para Análise Veterinária',     tags: ['YOLOv8', 'Visão Computacional', 'Deep Learning'] },
  'Assistente-Produtividade':    { title: 'Assistente de Produtividade',      tags: ['Python', 'Desktop', 'Tkinter'] },
  'Organizador-Automatico':      { title: 'Organizador Automático',           tags: ['Python', 'Automação', 'Watchdog'] },
  ...
  'Scrum_project_junior':        { title: 'Projeto Scrum Júnior',             tags: ['Scrum', 'Agile'] },
  'Scrum-project-junior':        { title: 'Projeto Scrum Júnior',             tags: ['Scrum', 'Agile'] },
  'Projeto_Pessoal_Gastos':      { title: 'Gestão Financeira Pessoal' },
  'Projeto-Pessoal-Gastos':      { title: 'Gestão Financeira Pessoal' },
  'Nexus_pim':                   { title: 'Nexus — Gestão de Produtos',       tags: ['Python', 'Streamlit'] },
  'Nexus-pim':                   { title: 'Nexus — Gestão de Produtos',       tags: ['Python', 'Streamlit'] },
  ...
  if (loading) return (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 22 }}>
      {Array.from({ length: 15 }).map((_, i) => (
        <div key={i} style={{ background: 'var(--surface)', border: '1px solid var(--border-2)', height: 220, opacity: 0.4, animation: 'pulse 1.5s ease-in-out infinite', animationDelay: `${i * 0.1}s` }} />
      ))}
    </div>
  );

  if (error) return (
    <div style={{ textAlign: 'center', padding: '3rem', color: 'var(--text-3)', fontFamily: 'monospace', fontSize: 12 }}>
      Não foi possível carregar os repositórios. <a href="https://github.com/Hiann?tab=repositories" target="_blank" rel="noreferrer" style={{ color: 'var(--accent)', textDecoration: 'none' }}>Ver no GitHub →</a>
    </div>
  );
    ...
    <>
      ...
      <div className="proj-grid-dyn">
        {repos.map((repo, i) => {
          ...
          const langColor = repo.language ? (LANG_COLOR[repo.language] ?? 'var(--text-3)') : null;
          const tags = meta.tags ?? (repo.topics?.length ? repo.topics.slice(0, 3) : repo.language ? [repo.language] : []);
          const index = String(i + 1).padStart(2, '0');

          return (
            <Reveal key={repo.id} delay={Math.min(i, 5) * 80}>
              <a href={repo.html_url} target="_blank" rel="noreferrer" className="card" style={{ display: 'flex', flexDirection: 'column', height: '100%', textDecoration: 'none', cursor: 'pointer' }}>
                <div className="card-accent-top" />
                <div className="proj-header">
                  <div className="proj-header-left">
                    <div className="proj-gh-logo">
                      <svg width="19" height="19" viewBox="0 0 24 24" fill="var(--text-3)">
                ...
                <div className="proj-body">
                  <div className="proj-title">{meta.title ?? repo.name.replace(/-/g, ' ').replace(/_/g, ' ')}</div>
                  <p className="proj-desc">{meta.desc ?? repo.description ?? 'Repositório disponível no GitHub.'}</p>
                  {tags.length > 0 && (
                  ...
                  <div className="proj-footer">
                    ...
                    {repo.forks_count > 0 && (
                      <span className="proj-stat">
                        <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><circle cx="12" cy="18" r="3"/><circle cx="6" cy="6" r="3"/><circle cx="18" cy="6" r="3"/><path d="M18 9v2a2 2 0 01-2 2H8a2 2 0 01-2-2V9"/><line x1="12" y1="12" x2="12" y2="15"/></svg>
                        {repo.forks_count}
                      </span>
                    )}
                  </div>
      ...
      <div style={{ textAlign: 'center', marginTop: '2.5rem' }}>
        <a href="https://github.com/Hiann?tab=repositories" target="_blank" rel="noreferrer"
          style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '10px 22px', border: '1px solid var(--border-2)', color: 'var(--text-2)', textDecoration: 'none', fontFamily: 'monospace', fontSize: 11, fontWeight: 700, letterSpacing: '0.15em', textTransform: 'uppercase', transition: 'border-color .2s, color .2s' }}
          ...
          onMouseLeave={e => { (e.currentTarget as HTMLAnchorElement).style.borderColor = 'var(--border-2)'; (e.currentTarget as HTMLAnchorElement).style.color = 'var(--text-2)'; }}>
          Ver todos no GitHub
...
function NavLinks({ NAV, section, go }: {
  ...
  return (
    <div className="nav-links" ref={containerRef}>
      <div className="nav-indicator" style={{ left: ind.left, width: ind.width }} />
      {NAV.map((l, i) => (
        <button key={l.id}
...
export default function Portfolio() {
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      for (const id of ['home', 'sobre', 'experiencia', 'projetos', 'skills', 'contato'].reverse()) {
        const el = document.getElementById(id);
        if (el && currentY >= el.offsetTop - 130) { setSection(id); break; }
      }
    };
  ...
  const NAV = [
    ...
    { id: 'experiencia', label: 'Experiência' }, { id: 'projetos', label: 'Projetos' },
    { id: 'skills', label: 'Skills' }, { id: 'contato', label: 'Contato' },
  ];
  const SKILLS = [
      ...
      <div className="root" suppressHydrationWarning>
        <style>{`
          ...
          *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
          ...
          .mobile-menu-btn { display: none; background: none; border: none; cursor: pointer; color: var(--text-1); font-size: 18px; padding: 4px; transition: color .2s; }
          .mobile-menu-btn:hover, .mobile-menu-btn:active { color: var(--accent); }
          
          .mobile-menu {
            background: var(--nav-bg); backdrop-filter: blur(12px); border-top: 1px solid var(--border-2); 
            padding: 1.5rem 2rem; display: flex; flex-direction: column; gap: 16px;
            animation: slideDown 0.3s cubic-bezier(0.16, 1, 0.3, 1) forwards; box-shadow: 0 10px 30px rgba(0,0,0,0.3);
          }
          @keyframes slideDown { from { opacity: 0; transform: translateY(-10px); } to { opacity: 1; transform: translateY(0); } }
          .mobile-nav-link {
            position: relative; background: none; border: none; cursor: pointer; color: var(--text-3);
            font-family: 'JetBrains Mono', monospace; font-size: 13px; font-weight: 600;
            letter-spacing: .2em; text-transform: uppercase; text-align: left;
            padding: 12px 0 12px 16px; border-bottom: 1px solid var(--border);
            transition: color .2s, transform .2s ease; overflow: hidden;
          }
          .mobile-nav-link::before {
            content: ''; position: absolute; left: 0; top: 50%; transform: translateY(-50%);
            width: 4px; height: 0; background: var(--accent); transition: height 0.2s ease;
          }
          .mobile-nav-link:hover, .mobile-nav-link:active { color: var(--text-1); transform: translateX(4px); }
          .mobile-nav-link:hover::before, .mobile-nav-link:active::before { height: 60%; }
          .mobile-nav-link.active { color: var(--text-1); font-weight: 700; }
          .mobile-nav-link.active::before { height: 100%; box-shadow: 0 0 8px var(--accent); }
          ...
          .hero { position:relative; z-index:10; min-height:100vh; display:flex; align-items:center; padding:0 2rem; max-width:1280px; margin:0 auto; }
          .hero-grid { width:100%; display:grid; grid-template-columns:7fr 5fr; gap:3rem; padding-top:80px; padding-bottom:60px; align-items:start; }
          ...
          .hero-bio { color:var(--text-2); max-width:500px; line-height:1.8; font-size:14px; border-left:3px solid var(--accent); padding-left:1.25rem; }
          ...
          .hero-stats { display:flex; gap:36px; padding-top:16px; border-top:1px solid var(--border-2); }
          ...
          .terminal { background:var(--surface); border:1px solid var(--border-2); padding:1.25rem; font-size:12.5px; transition:background .35s ease, border-color .35s ease; }
          ...
          .terminal-output { color:var(--text-2); padding-left:.875rem; margin-top:2px; }
          ...
          .section { position:relative; z-index:10; max-width:1280px; margin:0 auto; padding:7rem 2rem; }
          .section-sm { padding-top:2rem; padding-bottom:5rem; }
          ...
          .contact-reveal-right { display:flex; flex-direction:column; justify-content:flex-end; height:100%; }
          ...
          /* ── RESPONSIVE (Mágica da Centralização Mobile) ───────────────── */
          @media (max-width: 900px) {
            .hero-grid, .about-grid, .projects-grid, .skills-grid { grid-template-columns:1fr !important; }
            ...
            .navbar-inner { grid-template-columns: auto 1fr auto !important; direction: rtl; }
            .navbar-inner > * { direction: ltr; }
            .logo { border-right: none !important; padding-right: 0 !important; }
        ...
        <div className="dot-nav">
          {NAV.map(l => <button key={l.id} title={l.label} className={`dot-btn${section === l.id ? ' active' : ''}`} onClick={() => go(l.id)} />)}
        </div>
        ...
        <nav className="navbar">
          <div className="navbar-inner">
            ...
            <button className="logo" onClick={() => go('home')}>
              ...
              <span className="logo-text">DEV<span className="logo-accent">.</span>SYS</span>
              <div className="logo-meta">
                <div className="logo-meta-bot">
                  sec&nbsp;
                  <span className="logo-meta-bot-num">
                    {String(NAV.findIndex(l => l.id === section) + 1).padStart(2,'0')}
                  </span>
            ...
            <div className="mobile-menu">
              {NAV.map(l => (
                <button 
                  ...
                  onClick={() => { go(l.id); setMobileOpen(false); }}
                >
                  {l.label}
                </button>
              ))}
              <div style={{ paddingTop: '8px', borderTop: '1px solid var(--border-2)' }}>
                <ThemeToggle dark={dark} onToggle={() => toggleTheme()} />
              </div>
            </div>
          )}
            ...
            <div className="hero-left">
              <Reveal>
                <div className="status-badge">
                  <span className="status-dot-wrap"><span className="status-ping" /><span className="status-solid" /></span>
                  <span className="status-text"> Python Dev · Backend · IA</span>
                </div>
              </Reveal>
              <Reveal delay={100}>
                <h1 className="hero-name">
                  <GlitchText text="Hiann" /><br />
                  <span className="hero-name-dim"><GlitchText text="Alexander" dim /></span>
                </h1>
              </Reveal>
              <Reveal delay={180}>
                <div><TypeWriter words={['Programador Python', 'Dev Backend', 'Inteligência Artificial', 'Visão Computacional']} /></div>
              </Reveal>
              <Reveal delay={250}>
                <p className="hero-bio">Estudante de Sistemas de Informação com atuação em programação e análise de sistemas. Experiência em Python, Java e Web com integração a bancos de dados SQL e NoSQL, metodologias ágeis (Scrum) e vivência acadêmica em Deep Learning e Visão Computacional.</p>
              </Reveal>
              <Reveal delay={320}>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 14 }}>
                  <a href="/Curriculo_Hiann_Alexander.pdf" download className="btn-primary">
                    <svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path strokeLinecap="square" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/></svg>
                    Baixar CV
                  </a>
                  <button className="btn-outline" onClick={() => go('contato')}>Entrar em contato →</button>
                </div>
              </Reveal>
              <Reveal delay={400}>
                <div className="hero-stats">
                  {[{ v: 10, s: '+', l: 'Projetos' }, { v: 2, s: ' anos', l: 'Experiência' }, { v: 5, s: '+', l: 'Tecnologias' }].map(s => (
                    <div key={s.l}>
                      <div className="stat-num"><Counter target={s.v} suffix={s.s} /></div>
            ...
            <div className="hero-right">
              <Reveal delay={150}>
                <div className="profile-box">
                  ...
                  <img className="profile-img" src="/profile.jpeg" alt="Hiann Alexander" />
                  ...
                  <div className="profile-info">
                    ...
                    <div className="profile-status">
                      ...
                      <span className="profile-status-text">Disponível para projetos</span>
                    </div>
                  </div>
                </div>
              </Reveal>
              <Reveal delay={300}><Terminal /></Reveal>
            </div>
          </div>
        </section>
            ...
            <div className="marquee-inner">
              {[0, 1].map(n => (
                <span key={n} style={{ display: 'flex', gap: 44 }}>
                  {['Python', 'Machine Learning', 'Deep Learning', 'YOLOv8', 'SQL Server', 'MongoDB', 'Java', 'Git', 'Scrum', 'Visão Computacional', 'Automação'].map(t => (
                    <span key={t} className="marquee-item"><span className="marquee-dot" />{t}</span>
                  ))}
                </span>
              ))}
            </div>
        ...
        <section id="sobre" className="section">
          <Reveal><div className="slabel"><div className="slabel-line" /><span className="slabel-text">01 — Sobre</span></div></Reveal>
          <div className="card">
            ...
            <div className="about-box">
              ...
              <div className="about-grid">
                <Reveal>
                  <div>
                    <h3 className="about-title">A Arquitetura<br />por Trás do Código</h3>
                    <p className="about-p">Estudante de Sistemas de Informação com atuação em programação e análise de sistemas. Experiência prática em projetos acadêmicos e estágio utilizando Python, Java e desenvolvimento Web (HTML/CSS), com integração a bancos de dados como SQL Server, MySQL e MongoDB.</p>
                    <p className="about-p">Atua na manutenção de sistemas, correção de erros e implementação de funcionalidades seguindo boas práticas de código e metodologias ágeis (Scrum). Possui vivência acadêmica em Inteligência Artificial, desenvolvendo modelos de Deep Learning e Visão Computacional.</p>
                ...
                <Reveal delay={120}>
                  <div>
                    <p className="about-p">Diferencial técnico na pesquisa e aplicação de algoritmos avançados — Deep Learning e Visão Computacional com experiência acadêmica real.</p>
                    <p className="about-stack-title">Stack Principal</p>
                    {SKILLS.map(s => <SkillBar key={s.name} {...s} />)}
                    <div className="tag-list">
                      {['Python', 'Java', 'SQL Server', 'MySQL', 'MongoDB', 'YOLO', 'Scrum', 'Git','HTML/CSS'].map(t => <span key={t} className="tag">{t}</span>)}
                    </div>

                    <div style={{ marginTop: '2rem' }}>
                      <p className="about-stack-title">Formação</p>
                      {[
                        { course: 'Sistemas de Informação', institution: 'IF Goiano – Urutaí', period: 'Concluído: 2026' },
                        { course: 'Técnico em Informática', institution: 'IF Goiano – Urutaí', period: 'Concluído: 2019' },
                      ].map(f => (
                        <div key={f.course} style={{ marginBottom: '0.75rem', paddingBottom: '0.75rem', borderBottom: '1px solid var(--border)' }}>
                          <div style={{ fontSize: '13px', fontWeight: 700, color: 'var(--text-1)', fontFamily: "'Space Grotesk', sans-serif" }}>{f.course}</div>
                          <div style={{ fontSize: '11px', color: 'var(--accent)', marginTop: '2px' }}>{f.institution}</div>
                          <div style={{ fontSize: '10px', color: 'var(--text-3)', letterSpacing: '0.1em', marginTop: '2px' }}>{f.period}</div>
                        </div>
                      ))}
                    </div>
        ...
        <section id="experiencia" className="section section-sm">
          <Reveal><div className="slabel"><div className="slabel-line" /><span className="slabel-text">02 — Experiência</span></div></Reveal>
          <div className="timeline">
            <div className="timeline-dot"><div className="timeline-dot-inner" /></div>
            <Reveal>
              <div className="card exp-card">
                <div className="corner tl" style={{ borderColor: `rgba(var(--accent-rgb),.3)` }} />
                ...
                {[
                  ...
                  <div key={i} className="exp-item"><span className="exp-bullet" /><p className="exp-text">{t}</p></div>
                ))}
              </div>
            </Reveal>
          </div>
        ...
        <section id="projetos" className="section section-sm">
          <Reveal><div className="slabel"><div className="slabel-line" /><span className="slabel-text">03 — Projetos</span></div></Reveal>
          <GitHubProjects />
        </section>

        <section id="skills" className="section section-sm">
          <Reveal><div className="slabel"><div className="slabel-line" /><span className="slabel-text">04 — Competências</span></div></Reveal>
          <div className="skills-grid">
            {[
              ...
              },
              {
                icon: '◈',
                title: 'Dados & IA',
                items: ['SQL Server / MySQL', 'MongoDB', 'Deep Learning', 'Visão Computacional (YOLO)', 'Coleta e Rotulagem de Dados'],
              },
              {
                icon: '◻',
                title: 'Ferramentas & Métodos',
                items: ['Git / GitHub', 'Scrum / Agile', 'Windows', 'Debugging & Code Review', 'Documentação Técnica'],
              },
            ].map((c, i) => (
        ...
        <section id="contato" className="section" style={{ borderTop: '1px solid var(--border-2)' }}>
          <Reveal><div className="slabel"><div className="slabel-line" /><span className="slabel-text">05 — Contato</span></div></Reveal>
          <div className="contact-grid">
            
            <Reveal className="contact-reveal-left">
              <div className="contact-left-wrapper">
                <h3 className="contact-title">Vamos construir<br /><span className="contact-title-accent">algo juntos</span></h3>
                <p className="contact-desc">Disponível para projetos de IA, automações, arquiteturas de sistema ou oportunidades de estágio/trabalho.</p>
                  ...
                  {[
                    {
                      ...
                      icon: (
                        <svg width="20" height="16" viewBox="0 0 24 18" fill="none">
                          <rect x=".5" y=".5" width="23" height="17" rx="1.5" stroke="currentColor" strokeOpacity=".3"/>
                          <path d="M1 1.5L12 10L23 1.5" stroke="#ea4335" strokeWidth="1.5" fill="none" strokeLinecap="round"/>
                          <line x1="1" y1="1.5" x2="1" y2="17" stroke="#34a853" strokeWidth="1.5"/>
                          <line x1="23" y1="1.5" x2="23" y2="17" stroke="#fbbc05" strokeWidth="1.5"/>
                        </svg>
                      ),
                    },
                    {
                      ...
                      stripe: '#22c55e',
                      icon: (
                        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" style={{ color: '#22c55e' }}>
                    ...
                    },
                    {
                      ...
                      stripe: 'var(--text-3)',
                      icon: (
                        <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" style={{ color: 'var(--text-3)' }}><path d="M12 .297c-6.63 0-12 5.373-12 12 0 5.303 3.438 9.8 8.205 11.385.6.113.82-.258.82-.577 0-.285-.01-1.04-.015-2.04-3.338.724-4.042-1.61-4.042-1.61C4.422 18.07 3.633 17.7 3.633 17.7c-1.087-.744.084-.729.084-.729 1.205.084 1.838 1.236 1.838 1.236 1.07 1.835 2.809 1.305 3.495.998.108-.776.417-1.305.76-1.605-2.665-.3-5.466-1.332-5.466-5.93 0-1.31.465-2.38 1.235-3.22-.135-.303-.54-1.523.105-3.176 0 0 1.005-.322 3.3 1.23.96-.267 1.98-.399 3-.405 1.02.006 2.04.138 3 .405 2.28-1.552 3.285-1.23 3.285-1.23.645 1.653.24 2.873.12 3.176.765.84 1.23 1.91 1.23 3.22 0 4.61-2.805 5.625-5.475 5.92.42.36.81 1.096.81 2.22 0 1.606-.015 2.896-.015 3.286 0 .315.21.69.825.57C20.565 22.092 24 17.592 24 12.297c0-6.627-5.373-12-12-12"/></svg>
                      ),
                    },
                    {
                      label: 'LinkedIn',
                    ...
                    <a key={c.label} href={c.href} target="_blank" rel="noreferrer" className="ccard">
                      <div className="ccard-stripe" style={{ background: c.stripe }} />
                      <div className="ccard-icon" style={{ background: 'var(--bg-alt)' }}>{c.icon}</div>
        ...
        <footer className="footer" style={{ textAlign: 'center' }}>
          <div className="footer-inner" style={{ justifyContent: 'center', width: '100%' }}>
            <p className="footer-copy" style={{ margin: '0 auto', textAlign: 'center' }}>
              © {new Date().getFullYear()} Hiann Alexander Mendes de Oliveira — Todos os direitos reservados
            </p>
          </div>
        </footer>
      </div>
